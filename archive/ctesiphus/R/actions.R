...
apply_scout <- function(state, sp_spent, target_hex) {
  ...
  if (!is.na(res$init_resource)) {
    if (res$init_resource == "supply")      m$supply_remaining[i]   <- res$init_amount
    if (res$init_resource == "intel")       m$intel_remaining[i]    <- res$init_amount
    if (res$init_resource == "campaign")    m$campaign_remaining[i] <- res$init_amount
    if (res$init_resource == "search_cost") m$search_cost[i]        <- res$init_amount
  }
  state$map <- m
  ...
  list(action = "scout", ok = TRUE, reason = "",
       ...
       log = sprintf("Scouted hex %s -> %s / %s%s", target_hex,
                     ...
                     if (!is.na(res$init_resource)) sprintf(" (%s %d)", res$init_resource, res$init_amount) else ""),
       needs_input = NULL)
}
